// --- Core Microbial Strategy Simulation (v28d - CSS Layout Fix) ---
// Features: UI layout managed by CSS Flexbox in #ui-panel.
// ... (All other features remain as in v28) ...
// ---
...
// --- MODEL PARAMETERS ---
let PARAMS = {
    initialP: 0.5, mutationProbability: 0.01, K_G: 0.5, K_H: 0.5, h_max: 1.0, galactoseEfficiencyFactor: 0.85,
    minColonizationFitness: 0.05, colonizationPressureFactor: 0.8, maxColonizationProbability: 0.95,
    gradientMode: "smooth_radial", gradientCenter: 0.25, gradientTransitionWidth: 0.5,
    gaussianSourceQ: 0, gaussianSourceR: 0, gaussianWidth: 15, gaussianPeakG: 1.0, gaussianPeakH: 0.0, gaussianBaseG: 0.0, gaussianBaseH: 0.2,
...
// Removed uiPanelWidth global variable
...
function setup() {
    ...
    // *** Canvas width no longer includes UI panel ***
    let totalCanvasHeight = Math.max(canvasHeight, 600); // Min height for canvas itself
    ...
    noiseDetail(4, 0.5);
    createUI(); // Create UI elements within #ui-panel
    initGrid();
...
function draw() {
    ...
    if (frameCount % drawInterval === 0 || !start || showGradientOverlay) {
        // Clear only the canvas background (not the whole page)
        clear(); // Use clear() for the p5 canvas
        push(); translate(gridOriginX, gridOriginY);
        image(nutrientBuffer, -bufferWidth / 2, -bufferHeight / 2);
        if (!showGradientOverlay) {
            if (start) { drawLiveCells(this); }
            else { if (finalStateRendered) image(finalStateBuffer, -bufferWidth / 2, -bufferHeight / 2); else drawLiveCells(this); }
        }
        pop();
    }
    // UI updates are separate from canvas drawing now
...
function calculateBaseGradientValue(mode, q, r, R, pixelPos, minPixelX, maxPixelX, minPixelY, maxPixelY) {
    ...
    } = PARAMS; // Destructure needed params

    let baseG = 0.5, baseH = 0.5; // Default uniform mix

    if (mode === "smooth_radial") {
        let d = axialDistance(q, r, 0, 0);
        let maxDist = R > 0 ? R * 1.01 : 1; // Approximate max distance
        let d_norm = d / maxDist;
        let edge0 = gradientCenter - gradientTransitionWidth / 2;
        let edge1 = gradientCenter + gradientTransitionWidth / 2;
        let transitionFactor = smoothstep(edge0, edge1, d_norm);
        baseG = 1.0 - transitionFactor; baseH = transitionFactor;
        ...
        let d = axialDistance(q, r, gaussianSourceQ, gaussianSourceR);
        let distFactor = gaussianFalloff(d, gaussianWidth);
        baseG = gaussianBaseG + (gaussianPeakG - gaussianBaseG) * distFactor;
        baseH = gaussianBaseH + (gaussianPeakH - gaussianBaseH) * distFactor;
...
function initGrid() {
    start = false; loop(); lastStepBirths = 1; finalStateRendered = false; showGradientOverlay = false; // Reset overlay view
    if (uiElements.gradientOverlayBtn) uiElements.gradientOverlayBtn.html('Show Gradient'); // Reset button text
    ...
    // Calculate coordinate ranges needed for various gradient types
    ...
    for (let q = -R; q <= R; q++) {
        ...
        for (let r = r1; r <= r2; r++) {
            const coordStr = `${q},${r}`; const pixelPos = hexToPixel(q, r);
            const { gradientMode, noiseScale, noiseIntensityG, noiseIntensityH, noiseBaseGradient } = PARAMS;
            let gVal = 0.5, hVal = 0.5; // Defaults

            // Calculate base value using helper function
            let baseValues = calculateBaseGradientValue(gradientMode === 'noisy' ? noiseBaseGradient : gradientMode, q, r, R, pixelPos, minPixelX, maxPixelX, minPixelY, maxPixelY);
            gVal = baseValues.g;
            hVal = baseValues.h;
            ...
            if (gradientMode === 'noisy') {
                ...
                let noiseValH = noise(q * noiseScale + 100, r * noiseScale + 100, 0.2); // Offset noise space
            ...
            drawHexagon(nutrientBuffer, q, r, hexColor, null); // No stroke for bg buffer
        }
    }
    nutrientBuffer.pop();
    console.log(`Grid Initialized. Grad: ${PARAMS.gradientMode}. Frontier Cleared.`);
    noLoop(); // Pause after reset draw
}


// =============================================================================
// --- Simulation Step Logic --- (v27 - Collective Colonization Pressure)
// =============================================================================
function stepSimulation() {
    ...
    simulationSteps++; const { K_G, h_max, K_H, mutationProbability, galactoseEfficiencyFactor, minColonizationFitness, colonizationPressureFactor, maxColonizationProbability } = PARAMS;
    ...
    for (const coordStr of frontierSet) { if (!liveCells.has(coordStr)) continue; const cell = liveCells.get(coordStr); const G = glucose.get(coordStr) || 0; const H = galactose.get(coordStr) || 0; let S = 0; const p_threshold = cell.p; const ratio = (G > epsilon) ? H / G : Infinity; const prefersGalactose = (ratio >= p_threshold && H > epsilon) || (G <= epsilon && H > epsilon); if (prefersGalactose) { let s_potential_gal = h_max * (H / (K_H + H)); let eff = galactoseEfficiencyFactor * (1.0 - p_threshold); S = s_potential_gal * eff; } else if (G > epsilon) { S = G / (K_G + G); } S = Math.max(0, S); frontierFitness.set(coordStr, S); const [q, r] = coordStr.split(',').map(Number); let hasEmpty = false; for (const dir of axialDirections) { const nq = q + dir.q; const nr = r + dir.r; const nCoordStr = `${nq},${nr}`; if (glucose.has(nCoordStr) && !liveCells.has(nCoordStr)) { hasEmpty = true; break; } } if (hasEmpty) nextFrontierSet.add(coordStr); }
...
function createUI() {
    ...
    const createSliderRow = (label, paramKey, minVal, maxVal, stepVal, formatDigits, controlGroupArray) => {
        ...
        let slider = createSlider(minVal, maxVal, PARAMS[paramKey], stepVal).parent(rowDiv)
            .input(() => { // Input handler
                ...
                select('#' + paramKey + 'ValueDisplay').html(nf(slider.value(), 0, formatDigits));
                // Reset grid only if gradient param changed AND paused
                if (paramKey.includes('gaussian') || paramKey.includes('gradient') || paramKey.includes('noise')) {
                    if (!start) { console.log(`Gradient parameter ${paramKey} changed, resetting...`); initGrid(); redraw(); updateUIParamDisplays(); }
                    else { console.log(`Gradient parameter ${paramKey} changed, but sim running. Reset needed to apply.`); }
                }
            });
        createSpan(nf(PARAMS[paramKey], 1, formatDigits)).parent(rowDiv)
            .id(paramKey + 'ValueDisplay'); // Value Display

        uiElements[paramKey + 'Slider'] = slider;
        uiParamDisplays[paramKey] = select('#' + paramKey + 'ValueDisplay');

        if (controlGroupArray) { controlGroupArray.push(rowDiv); }
        return rowDiv;
    };

    const createSectionHeader = (text, controlGroupArray) => {
        let headerP = createP(`<b>${text}</b>`).parent('#ui-panel');
        if (controlGroupArray) { controlGroupArray.push(headerP); }
        return headerP;
    };

    // --- Create UI Elements (Parented to #ui-panel, NO position()) ---
    ...
    uiElements.gradientSelector = createSelect().parent(gradTypeRowDiv)
        .changed(() => { PARAMS.gradientMode = uiElements.gradientSelector.value(); console.log("Gradient changed..."); if (!start) { initGrid(); redraw(); updateUIParamDisplays(); } else { console.log("Reset needed to apply gradient change."); } });
    ['smooth_radial', 'smooth_linear_h', 'smooth_linear_v', 'gaussian_source', 'noisy', 'uniform_g', 'uniform_h', 'uniform_mix'].forEach(opt => uiElements.gradientSelector.option(opt));
    ...
    createSliderRow("Gal Max Growth (h<sub>max</sub>):", 'h_max', 0, 1.5, 0.01, 2);
    createSliderRow("Gal Max Efficiency:", 'galactoseEfficiencyFactor', 0, 1, 0.01, 2);
...
function updateUIParamDisplays() {
    ...
    for (const key in uiParamDisplays) {
        if (PARAMS.hasOwnProperty(key) && uiParamDisplays[key]) {
            ...
            else if (key === 'gaussianSourceQ' || key === 'gaussianSourceR') digits = 0;
    ...
    for (const groupName in uiElements.controlGroups) {
        ...
        uiElements.controlGroups[groupName].forEach(element => {
            if (element) { // Ensure element (div or p) exists
...
function mousePressed() {
    ...
    if (mouseX >= 0 && mouseX <= width && mouseY >= 0 && mouseY <= height) {
        ...
        // Check if the click was within the UI panel area (assuming it's to the right)
        // Use canvasWidth which represents the grid area width.
        if (mouseX > canvasWidth) {
            // Click is likely in the UI panel, ignore for planting.
            return;
        }

        // --- Calculate click coordinates relative to the grid's visual center ---
        // p5's mouseX/mouseY are relative to the canvas top-left (0,0).
        // The grid drawing is centered using translate(gridOriginX, gridOriginY).
        // So, we subtract the origin offset from the mouse coordinates.
        let clickXRelativeToGridOrigin = mouseX - gridOriginX;
        let clickYRelativeToGridOrigin = mouseY - gridOriginY;

        // Convert the relative pixel coordinates to hex coordinates
        ...
        // Add a small buffer (+2) to allow clicking slightly outside the drawn edge
        if (axialDistance(hexCoords.q, hexCoords.r, 0, 0) <= maxInitRadius + 2) {
            // Plant the patch centered at the calculated hex coordinate
            plantPatch(hexCoords.q, hexCoords.r, 2); // Plant a 2-radius patch
            if (!start) {
                redraw(); // Force redraw if paused to show the new patch
            }
        } else {
            console.log("Click outside defined grid radius.");
        }
    }
...
function keyPressed() {
    if (key === ' ') { toggleSimulation(); return false; } // Prevent spacebar scroll
}
// =============================================================================